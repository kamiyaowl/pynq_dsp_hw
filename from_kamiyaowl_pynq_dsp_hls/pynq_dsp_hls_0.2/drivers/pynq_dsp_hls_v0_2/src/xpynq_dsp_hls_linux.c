...
typedef struct {
    ...
    XPynq_dsp_hls_uio_map maps[ MAX_UIO_MAPS ];
} XPynq_dsp_hls_uio_info;

/***************** Variable Definitions **************************************/
static XPynq_dsp_hls_uio_info uio_info;
...
static int uio_info_read_name(XPynq_dsp_hls_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XPynq_dsp_hls_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XPynq_dsp_hls_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XPynq_dsp_hls_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XPynq_dsp_hls_Initialize(XPynq_dsp_hls *InstancePtr, const char* InstanceName) {
	XPynq_dsp_hls_uio_info *InfoPtr = &uio_info;
