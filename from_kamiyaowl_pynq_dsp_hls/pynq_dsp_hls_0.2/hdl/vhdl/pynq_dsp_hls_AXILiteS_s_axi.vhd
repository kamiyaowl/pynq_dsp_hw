...
architecture behave of pynq_dsp_hls_AXILiteS_s_axi is
    ...
    signal int_configReg_d1    : UNSIGNED(31 downto 0);
    ...
    end component pynq_dsp_hls_AXILiteS_s_axi_ram;
...
port map (
     ...
     ce0      => int_configReg_ce0,
     we0      => int_configReg_we0,
     be0      => int_configReg_be0,
     d0       => int_configReg_d0,
     q0       => int_configReg_q0,
     clk1     => ACLK,
     address1 => int_configReg_address1,
     ce1      => int_configReg_ce1,
     we1      => int_configReg_we1,
     be1      => int_configReg_be1,
     d1       => int_configReg_d1,
     q1       => int_configReg_q1);
...
-- ----------------------- AXI READ ----------------------
    ...
    RRESP   <= "00";  -- OKAY
    RVALID_t  <= '1' when (rstate = rddata) and (int_configReg_read = '0') else '0';
    RVALID    <= RVALID_t;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                ...
                elsif (int_configReg_read = '1') then
                    rdata_data <= int_configReg_q1;
                end if;
            end if;
        end if;
    end process;
...
-- ----------------------- Register logic ----------------
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_BASEPHYSADDR_V_DATA_0) then
                    int_basePhysAddr_V(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_basePhysAddr_V(31 downto 0));
                end if;
            end if;
        end if;
    end process;
...
-- ----------------------- Memory logic ------------------
    ...
    int_configReg_d1     <= UNSIGNED(WDATA);

    process (ACLK)
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_configReg_read <= '0';
            elsif (ACLK_EN = '1') then
                if (ar_hs = '1' and raddr >= ADDR_CONFIGREG_BASE and raddr <= ADDR_CONFIGREG_HIGH) then
                    int_configReg_read <= '1';
                else
                    int_configReg_read <= '0';
                end if;
            end if;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                ...
                if (aw_hs = '1' and UNSIGNED(AWADDR(ADDR_BITS-1 downto 0)) >= ADDR_CONFIGREG_BASE and UNSIGNED(AWADDR(ADDR_BITS-1 downto 0)) <= ADDR_CONFIGREG_HIGH) then
...
entity pynq_dsp_hls_AXILiteS_s_axi_ram is
    generic (
        BYTES   : INTEGER :=4;
        DEPTH   : INTEGER :=256;
        AWIDTH  : INTEGER :=8);
...
end entity pynq_dsp_hls_AXILiteS_s_axi_ram;

architecture behave of pynq_dsp_hls_AXILiteS_s_axi_ram is
