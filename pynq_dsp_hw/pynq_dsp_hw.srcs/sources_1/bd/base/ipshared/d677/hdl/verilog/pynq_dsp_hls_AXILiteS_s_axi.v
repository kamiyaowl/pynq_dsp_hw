...
assign RVALID  = (rstate == RDDATA) & !int_configReg_read;
...
// int_basePhysAddr_V[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_basePhysAddr_V[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_BASEPHYSADDR_V_DATA_0)
            int_basePhysAddr_V[31:0] <= (WDATA[31:0] & wmask) | (int_basePhysAddr_V[31:0] & ~wmask);
    end
end
...
assign int_configReg_d1       = WDATA;
// int_configReg_read
always @(posedge ACLK) begin
    if (ARESET)
        int_configReg_read <= 1'b0;
    else if (ACLK_EN) begin
        if (ar_hs && raddr >= ADDR_CONFIGREG_BASE && raddr <= ADDR_CONFIGREG_HIGH)
            int_configReg_read <= 1'b1;
        else
            int_configReg_read <= 1'b0;
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (aw_hs && AWADDR[ADDR_BITS-1:0] >= ADDR_CONFIGREG_BASE && AWADDR[ADDR_BITS-1:0] <= ADDR_CONFIGREG_HIGH)
...
module pynq_dsp_hls_AXILiteS_s_axi_ram
#(parameter
    BYTES  = 4,
    DEPTH  = 256,
    AWIDTH = log2(DEPTH)
