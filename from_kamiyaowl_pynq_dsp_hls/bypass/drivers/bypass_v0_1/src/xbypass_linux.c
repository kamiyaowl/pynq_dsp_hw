...
typedef struct {
    ...
    XBypass_uio_map maps[ MAX_UIO_MAPS ];
} XBypass_uio_info;

/***************** Variable Definitions **************************************/
static XBypass_uio_info uio_info;
...
static int uio_info_read_name(XBypass_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XBypass_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XBypass_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XBypass_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XBypass_Initialize(XBypass *InstancePtr, const char* InstanceName) {
	XBypass_uio_info *InfoPtr = &uio_info;
